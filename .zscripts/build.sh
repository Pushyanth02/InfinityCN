...
BUILD_DIR="${BUILD_DIR:-$NEXTJS_PROJECT_DIR/.build}"
...
# 准备构建产物中的数据库：如果存在测试环境数据库则作为种子复制，
# 否则从 schema 全新创建。随后统一用 db:push 同步 schema，保证结构最新。
echo "🗄️  准备构建产物数据库..."
mkdir -p "$BUILD_DIR/db"
if [ -f "./db/custom.db" ]; then
    echo "  - 复制现有种子数据库 ./db/custom.db"
    cp -r ./db/. "$BUILD_DIR/db/"
else
    echo "  - 未找到 ./db/custom.db，将从 schema 创建全新数据库"
fi
