...
export function CompanionDrawer({
  ...
  return (
    ...
    >
      ...
      <div
        className={cx("flex flex-col h-full", tab !== "luma" && "hidden")}
        aria-hidden={tab !== "luma"}
      >
        <LumaChat doc={doc} chapterIndex={chapterIndex} selection={selection} />
      </div>
      <div
        className={cx("flex flex-col h-full", tab !== "ouro" && "hidden")}
        aria-hidden={tab !== "ouro"}
      >
        <OuroPanel doc={doc} chapterIndex={chapterIndex} />
      </div>
      <div
        className={cx("flex flex-col h-full", tab !== "ankaa" && "hidden")}
        aria-hidden={tab !== "ankaa"}
      >
        <AnkaaPanel doc={doc} />
      </div>
    </Sheet>
      ...
      {blocks.map((ln, i) => {
        if (ln.startsWith("> ")) {
          return (
            <blockquote
              key={i}
              className="border-l-2 border-gold-600/70 pl-3 my-1 italic opacity-90 font-literata leading-relaxed"
            >
              <InlineRich text={ln.slice(2)} />
            </blockquote>
          );
        }
        if (ln.startsWith("- ")) {
          return (
            <div key={i} className="flex gap-2.5">
              <span className="text-gold-500 shrink-0 leading-relaxed mt-0.5">
                ·
              </span>
...
function LumaChat({
  ...
  const chapterTitle =
    doc.contentJson.chapters[chapterIndex]?.title ?? "this chapter";
    ...
    return [
      {
        ...
        role: "assistant",
        at: Date.now(),
        text: `Hello — I’m Luma, reading “${doc.title}” beside you. Ask about this chapter, a passage, a word — or have me paint the scene.`,
      },
    ];
  });
  ...
  const append = useCallback((msgId: string, chunk: string) => {
    setMessages((m) =>
      m.map((x) => (x.id === msgId ? { ...x, text: x.text + chunk } : x)),
    );
  }, []);
  ...
  const send = useCallback(
    async (question: string) => {
      ...
      setMessages((m) => [
        ...m,
        userMsg,
        { id: botId, role: "assistant", text: "", at: Date.now() },
      ]);
      setBusy(true);
      ...
      try {
        ...
        const res = await askLumaStream(
          doc,
          chapterIndex,
          q,
          history,
          (chunk) => append(botId, chunk),
          ctrl.signal,
        );
        ...
        if (ctrl.signal.aborted) {
          setMessages((m) =>
            m.map((x) =>
              x.id === botId && !x.text ? { ...x, text: "_Stopped._" } : x,
            ),
          );
        } else {
          const msg =
            e instanceof AiUnavailable
              ? e.message
              : "Luma lost the thread for a moment. Try again?";
          setMessages((m) =>
            m.map((x) => (x.id === botId && !x.text ? { ...x, text: msg } : x)),
          );
    ...
    setMessages([
      {
        ...
        role: "assistant",
        at: Date.now(),
        text: `Fresh page. What shall we look at in “${chapterTitle}”?`,
      },
    ]);
    setModel(null);
    setLastOffline(null);
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="px-4 sm:px-5 py-3.5 border-b border-ink-700 flex items-center gap-3 shrink-0 bg-ink-875/50">
        <span
          className={cx(
            ...
            busy
              ...
              : "border-gold-700/60 bg-gold-500/10 text-gold-300",
          )}
        >
          L
          <span
            className={cx(
              "absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 rounded-full border-2 border-ink-875",
              busy ? "bg-warn-400 animate-pulse" : "bg-ok-500",
            )}
            aria-hidden
          />
        </span>
        <div className="min-w-0 flex-1">
          <p className="text-[13px] font-display text-mist-100 flex items-center gap-2 leading-tight">
            Luma
            {model ? (
              <span className="text-[10px] font-mono text-mist-500 truncate max-w-32">
                {model}
              </span>
            ) : (
              <Badge tone={serverOnline ? "gold" : "muted"}>
                {serverOnline ? "online" : "Anchor engine"}
              </Badge>
            )}
          </p>
          <p className="text-[11px] text-mist-500 truncate mt-0.5">
            grounded in “{chapterTitle}”
          </p>
        </div>
        <IconBtn label="Clear conversation" onClick={clearThread}>
          ...
          return m.role === "user" ? (
            ...
            <div key={m.id} className="group">
              <div className="rounded-2xl border border-ink-700 bg-ink-800/80 px-4 py-3.5 text-[13px] leading-relaxed text-mist-300">
                {!m.text ? (
                  <span
                    className="inline-flex items-center gap-1.5 text-mist-500 py-1"
                  ...
                  <>
                    ...
                    {streaming && (
                      <span
                        className="inline-block w-1.75 h-3.75 bg-gold-400/80 ml-0.5 align-[-2px] animate-pulse rounded-sm"
                        aria-hidden
                      />
                    )}
                  </>
                ...
                {m.text && !streaming && (
                  <>
                    ...
                    {lastOffline !== null && isLast && (
                      <span className="text-[10px] font-display text-mist-600 ml-auto">
                        {lastOffline ? "Anchor engine" : "live model"}
                      </span>
                    )}
                  </>
                )}
      ...
      {selection && !busy && (
        <div className="px-4 sm:px-5 pb-2 shrink-0">
          <button
            onClick={() =>
              void send(`Explain this passage: “${clipQuote(selection)}”`)
            }
            className="w-full flex items-center gap-2.5 rounded-xl border border-gold-700/50 bg-gold-500/7 px-3 py-2.5 text-left hover:bg-gold-500/13 hover:border-gold-600/70 transition-all press"
          >
            <Quote className="w-4 h-4 text-gold-400 shrink-0" />
            <span className="min-w-0 flex-1">
              <span className="block text-[10px] font-display uppercase tracking-[0.16em] text-gold-400">
                Ask about your selection
              </span>
              <span className="block text-xs text-mist-400 italic font-literata truncate mt-0.5">
                “{selection}”
              </span>
            </span>
          </button>
      ...
      >
        <div className="flex items-end gap-2 rounded-xl border border-ink-600 bg-ink-850 focus-within:border-gold-600 focus-within:shadow-[0_0_0_3px_var(--acc-soft)] transition-all p-2">
          <textarea
            ...
            aria-label="Ask Luma"
            className="flex-1 bg-transparent resize-none outline-none text-sm text-mist-100 placeholder:text-mist-600 max-h-35 px-2 py-1.5 leading-relaxed"
          />
          {busy ? (
            <Button
              type="button"
        ...
        <p className="text-[10px] text-mist-600 mt-1.5 px-1 hidden sm:block">
...
const OURO_TASKS: {
...
}[] = [
  {
    ...
    label: "Summary",
    icon: BookOpen,
    hint: "What happens, distilled",
  },
  {
    task: "quiz",
    label: "Quiz me",
    icon: HelpCircle,
    hint: "Sourced multiple-choice",
  },
  {
    task: "guide",
    label: "Study guide",
    icon: Compass,
    hint: "Objectives + close-reading steps",
  },
  {
    task: "themes",
    label: "Themes & cast",
    icon: Lightbulb,
    hint: "Motifs and their carriers",
  },
  {
    task: "vocab",
    label: "Vocabulary",
    icon: Languages,
    hint: "Words worth keeping",
  },
  {
    task: "essays",
    label: "Essay prompts",
    icon: PenLine,
    hint: "University-level questions",
  },
  {
    task: "full",
    label: "Full study set",
    icon: ListChecks,
    hint: "Everything in one pass",
  },
];
...
export function OuroPanel({
  ...
  const chapterTitle =
    doc.contentJson.chapters[chapterIndex]?.title ?? "this chapter";
  const scopeLabel =
    scope === "chapter" ? `“${chapterTitle}”` : "the whole text";
  ...
  const request = useCallback(
    async (task: OuroTask) => {
      ...
      setFeed((f) => [
        ...f,
        { key, task, scope: itemScope, status: "loading" },
      ]);
      try {
        const artifact = await getOuroArtifact(
          doc,
          itemScope === "chapter" ? chapterIndex : null,
          task,
        );
        setFeed((f) =>
          f.map((x) =>
            x.key === key ? { ...x, status: "done", artifact } : x,
          ),
        );
      } catch (e) {
        const msg =
          e instanceof AiUnavailable
            ? e.message
            : "Ouro couldn’t build that one. Try again?";
        setFeed((f) =>
          f.map((x) =>
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto px-4 sm:px-5 py-4 space-y-4">
        {feed.length === 0 && (
          <div className="text-center pt-6 pb-2">
            ...
            <p className="text-sm font-display text-mist-200">
              What do you need from {scopeLabel}?
            </p>
            <p className="text-xs text-mist-500 mt-1.5 max-w-xs mx-auto leading-relaxed">
              Tell me in the box below, or start with one of these — I’ll build
              exactly that, and cache it.
            </p>
          </div>
        )}
          ...
          <div className="grid grid-cols-2 gap-2.5">
            {OURO_TASKS.map((t) => (
              ...
              >
                ...
                <span className="block text-xs text-mist-200 font-display">
                  {t.label}
                </span>
                <span className="block text-[10px] text-mist-500 mt-0.5 leading-snug">
                  {t.hint}
                </span>
              </button>
            ))}
        ...
        {feed.map((item) => (
          <div
            key={item.key}
            className="rounded-xl border border-ink-700 bg-ink-800/80 overflow-hidden hover-lift"
          >
            <div className="flex items-center gap-2.5 px-4 py-3 border-b border-ink-700/70 bg-ink-875/60">
              <span className="text-[10px] font-display uppercase tracking-[0.16em] text-ouro-300">
                {TASK_TITLES[item.task]}
              </span>
              <span className="text-[10px] text-mist-600">
                · {item.scope === "chapter" ? "this chapter" : "whole text"}
              </span>
              {item.status === "done" && item.artifact && (
                <Badge
                  tone={item.artifact.offline ? "muted" : "ouro"}
                  className="ml-auto"
                >
                  {item.artifact.offline ? "LOA" : "model"}
                </Badge>
              )}
              {item.status === "loading" && (
                <Loader2 className="w-3.5 h-3.5 animate-spin text-ouro-400 ml-auto" />
              )}
            </div>
              ...
              {item.status === "error" && (
                <div>
                  ...
                  <Button
                    size="xs"
                    variant="outline"
                    onClick={() => void request(item.task)}
                  >
                    <RefreshCw className="w-3 h-3" />
                    Retry
                  </Button>
                ...
                <>
                  ...
                  {item.artifact.quiz && item.artifact.quiz.length > 0 && (
                    <div className="mt-4 pt-4 border-t border-ink-700/60">
                      <QuizBlock quiz={item.artifact.quiz} />
                    </div>
                  )}
                </>
              )}
...
function OuroAsk({ onAsk }: { onAsk: (task: OuroTask) => void }) {
  ...
  return (
    ...
    >
      <div className="flex items-end gap-2 rounded-xl border border-ink-600 bg-ink-850 focus-within:border-ouro-500 focus-within:shadow-[0_0_0_3px_rgb(109_132_232/0.12)] transition-all p-2">
        <input
          value={text}
        ...
        <Button
          ...
          variant="gold"
          size="sm"
          aria-label="Build study artifact"
          disabled={!text.trim()}
        >
...
function OuroSection({
  ...
  return (
    <section>
      <h3 className="text-[10px] font-display uppercase tracking-[0.2em] text-ouro-300 mb-2.5">
        {title}
      </h3>
      {children}
    </section>
...
function QuizBlock({ quiz }: { quiz: QuizQuestion[] }) {
  ...
  const correct = Object.entries(answers).filter(
    ([qi, a]) => (quiz[Number(qi)]?.answer ?? -1) === a,
  ).length;
  return (
    <OuroSection
      title={`Quiz — ${answered === 0 ? `${quiz.length} questions` : `${correct}/${answered} correct`}`}
    >
      <div className="space-y-3">
        {quiz.map((q, qi) => (
          <div key={qi} className="hairline rounded-lg bg-ink-875 p-3.5">
            <p className="text-[13px] text-mist-200 leading-relaxed mb-3">
              {q.q}
            </p>
                ...
                return (
                  <button
                    ...
                    className={cx(
                      ...
                      chosen === undefined &&
                        "border-ink-600 text-mist-300 hover:border-ouro-500 hover:text-ouro-300 hover:bg-ink-800",
                      chosen !== undefined &&
                        isAnswer &&
                        "border-ok-500/60 bg-ok-500/10 text-ok-400",
                      chosen !== undefined &&
                        isChosen &&
                        !isAnswer &&
                        "border-danger-500/60 bg-danger-500/10 text-danger-400",
                      chosen !== undefined &&
                        !isChosen &&
                        !isAnswer &&
            ...
            {answers[qi] !== undefined && (
              <p className="text-[11px] text-mist-500 mt-2.5 italic font-literata leading-relaxed">
                Source: “{q.why}”
              </p>
            )}
          </div>
...
const ANKAA_MODES: { id: AnkaaMode; label: string; hint: string }[] = [
  {
    id: "continue",
    label: "Continue",
    hint: "Pick up where the text left off",
  },
  ...
  { id: "whatif", label: "What if…", hint: "One hinge, another door" },
  {
    ...
    hint: "Fork the last turning point",
  },
  { id: "lore", label: "World lore", hint: "Histories the text only implies" },
  {
    id: "children",
    label: "For children",
    hint: "The same story, gentler hands",
  },
];
...
export function AnkaaPanel({ doc }: { doc: DocumentRow }) {
  ...
  const append = async () => {
    ...
    setAppending(true);
    try {
      const chapterTitle = result.title.includes("—")
        ? result.title.split("—").pop()!.trim()
        : result.title;
      await appendChapterToDocument(doc.id, chapterTitle, result.body);
      setAppended(true);
      toast(
        "success",
        `Added “${chapterTitle}” as chapter ${doc.chapterCount + 1} of “${doc.title}”.`,
      );
    } catch (e) {
      toast(
        "error",
        e instanceof Error ? e.message : "Couldn’t append the draft.",
      );
    } finally {
      setAppending(false);
    }
  ...
  const generate = async () => {
    ...
    enqueueJob({
      ...
      run: async (report) =>
        runAnkaaLong(
          ...
          (stepIdx, frac, words) => {
            ...
            setLive((l) =>
              l && l.status === "running" ? { ...l, stepIdx, words } : l,
            );
            void patchStory(id, {
              step: steps[stepIdx],
              progress: Math.round(((stepIdx + frac) / steps.length) * 100),
            });
          },
          undefined,
          runDepth,
      ...
      onDone: async (res) => {
        ...
        toast(
          "success",
          `Ankaa finished — ${res.words.toLocaleString()} words${res.offline ? " (Anchor engine)" : ""}.`,
        );
      },
      onError: async (err) => {
  ...
  const stepLabel = running
    ? steps[clamp(live?.stepIdx ?? 0, 0, steps.length - 1)]
    : null;
  ...
  return (
    <div className="flex flex-col h-full">
      {/* context strip */}
      <div className="px-4 sm:px-5 py-3.5 border-b border-ink-700 flex items-center gap-3 shrink-0 bg-ink-875/50">
        <span className="w-10 h-10 rounded-xl border border-ankaa-500/50 bg-ankaa-500/10 flex items-center justify-center font-garamond italic text-lg text-ankaa-300 shrink-0">
          A
        </span>
        <div className="min-w-0 flex-1">
          <p className="text-[13px] font-display text-mist-100 leading-tight">
            Ankaa · long-form
          </p>
          <p className="text-[11px] text-mist-500 truncate mt-0.5">
            writing from “{doc.title}” — runs in the background
          </p>
        </div>
        <Badge
          tone={
            activeModelFor("ankaa").startsWith("meridian/") || !aiConfigured()
              ? "muted"
              : "ok"
          }
        >
      ...
      <div className="flex-1 overflow-y-auto px-4 sm:px-5 py-4 space-y-5">
        {/* approach */}
        <div>
          <p className="text-[10px] font-display uppercase tracking-[0.2em] text-mist-500 mb-2.5">
            Approach
          </p>
          <div
            ...
            {ANKAA_MODES.map((m) => (
              <button
                ...
                className={cx(
                  ...
                  mode === m.id
                    ? "border-ankaa-500/70 bg-ankaa-500/9 shadow-[0_0_0_3px_rgb(219_129_76/0.10)]"
                    : "border-ink-600 hover:border-ink-500 bg-ink-875 hover:bg-ink-800",
                )}
              >
                <span
                  className={cx(
                    "block text-xs font-display",
                    mode === m.id ? "text-ankaa-300" : "text-mist-200",
                  )}
                >
                  {m.label}
                </span>
        ...
        <div>
          <div className="flex items-center justify-between gap-2 mb-2">
            <p className="text-[10px] font-display uppercase tracking-[0.2em] text-mist-500">
              What should it be about?
            </p>
            <Badge
              tone="ankaa"
              title={`${ankaaSectionsFor(depth)} sections · ${depth}-form draft`}
            >
              {depth === "short"
                ? "Short-form"
                : depth === "medium"
          ...
          <textarea
            ...
            className="w-full rounded-xl border border-ink-600 bg-ink-850 px-3.5 py-3 text-sm text-mist-100 placeholder:text-mist-600 focus:border-ankaa-500 focus:shadow-[0_0_0_3px_rgb(219_129_76/0.12)] outline-none transition-all resize-y leading-relaxed"
          />
          <p className="text-[10px] text-mist-600 mt-1.5 leading-relaxed">
            Everything you name becomes canon — Ankaa weaves it through every
            section. Type “short scene…” to get a brief draft instead of a full
            chapter.
          </p>
        </div>
        ...
        {/* progress / result */}
        {running && live && (
          <div className="rounded-xl border border-ankaa-500/40 bg-ankaa-500/6 p-4">
            <div className="flex items-center gap-2.5 mb-2.5">
              <Loader2 className="w-4 h-4 animate-spin text-ankaa-400" />
              <p className="text-xs font-display text-ankaa-300">
                {stepLabel}…
              </p>
              <span className="ml-auto text-[11px] text-mist-400 tabular-nums">
                {live.words.toLocaleString()} words
              </span>
            </div>
            <Progress value={((live.stepIdx + 0.4) / steps.length) * 100} />
            <p className="text-[10px] text-mist-600 mt-2 leading-relaxed">
              Visible in the header job tray — safe to keep reading meanwhile.
            </p>
          </div>
        )}

        {live?.status === "failed" && (
          <div className="rounded-xl border border-danger-500/40 bg-danger-500/6 p-4">
            <p className="text-xs text-danger-400 mb-2.5 leading-relaxed">
              {live.error}
            </p>
        ...
        {live?.status === "done" && result && (
          <div className="rounded-xl border border-ok-500/40 bg-ok-500/6 p-4">
            <p className="text-sm font-display text-mist-100 mb-0.5">
              {result.title}
            </p>
            <p className="text-[11px] text-mist-500 mb-3.5">
              {result.words.toLocaleString()} words
              {result.offline ? " · Anchor engine" : ""} · written from this
              book’s context
            </p>
            <div className="flex flex-wrap gap-2">
              ...
              >
                ...
                {appended ? "Added to this book" : "Continue this book"}
              </Button>
              <Button
                size="sm"
                variant="outline"
            ...
            {appended && (
              <p className="text-[10px] text-ok-400 mt-2.5 leading-relaxed">
                The draft is now chapter {doc.chapterCount + 1} of “{doc.title}”
                — find it in the chapter index (t) or keep reading.
              </p>
            )}
          </div>
        )}
        ...
        {!live && (
          <p className="text-[11px] text-mist-600 leading-relaxed">
            Ankaa outlines first, then writes section by section with a live
            word count. Length scales to your prompt — type “short scene…” for
            ~400 words, or leave it long for a full chapter (~2,500 with a
            model, ~1,800 with the Anchor engine).
          </p>
        )}
      </div>
            ...
            <div className="flex flex-wrap justify-between items-center gap-2 mt-5 pt-4 border-t border-ink-700">
              ...
              <div className="flex gap-2 flex-wrap">
                ...
                >
                  ...
                  {appended ? "Added" : "Continue this book"}
                </Button>
...
function moodGradient(mood: string): string {
  ...
  if (/dawn|morn|sun|gold|warm|amber|cream/.test(m))
    return "linear-gradient(135deg, #2a1f12, #6b4d2a 35%, #c39a45 80%, #f0d088)";
  if (/night|dark|midnight|shadow|gloom|moon/.test(m))
    return "linear-gradient(135deg, #06060c, #1a1a2e 45%, #16213e 90%)";
  if (/storm|rain|grey|gray|cold|winter|frost/.test(m))
    return "linear-gradient(135deg, #0d141b, #2c3e50 50%, #4a5a6e 95%)";
  if (/dusk|sunset|ember|red|fire|blood|crimson/.test(m))
    return "linear-gradient(135deg, #1a0808, #6b2e2a 40%, #c0392b 80%, #d97e4a)";
  if (/forest|green|nature|spring|moss|leaf/.test(m))
    return "linear-gradient(135deg, #0d1810, #2d4a3e 50%, #4a7050 95%)";
  if (/sea|ocean|water|aqua|river/.test(m))
    return "linear-gradient(135deg, #0a1620, #1e3a5f 50%, #4a90a4 95%)";
  if (/interior|indoors|lamp|candle|hearth/.test(m))
    return "linear-gradient(135deg, #1a1206, #3a2e1a 40%, #6b4d2a 80%, #c39a45)";
...
export function ScenesView({
    ...
    async (silent = false, salt = 0) => {
      ...
      try {
        ...
        if (!silent)
          toast(
            "success",
            `Cinematized “${chapter?.title}” into ${res.scenes.length} scene${res.scenes.length === 1 ? "" : "s"}${res.offline ? " (Anchor engine)" : ""}.`,
          );
      } catch (e) {
        if (stale()) return;
        toast(
          ...
          e instanceof AiUnavailable
            ? e.message
            : "Cinematization failed — try Regenerate; the Anchor engine will step in.",
        );
      } finally {
    ...
    async (ordinal: number) => {
      ...
      try {
        ...
        toast(
          ...
          e instanceof AiUnavailable
            ? e.message
            : "Couldn't regenerate that scene — try again.",
        );
      } finally {
  ...
  useEffect(() => {
    if (
      ...
      !autoGen.current.has(chapterIndex) &&
      !generating
    ) {
  ...
  const sourceChunkFor = useCallback(
    (scene: SceneDraft): number => {
      if (!chunks.length) return 0;
      const probe = (scene.title + " " + scene.body).toLowerCase();
      const words = Array.from(
        new Set(probe.split(/\W+/).filter((w) => w.length > 4)),
      ).slice(0, 24);
      let best = 0;
      let bestScore = -1;
      chunks.forEach((c, i) => {
  ...
  const headerActions = (
    <div className="flex items-center gap-2 flex-wrap">
        ...
        <Segmented
          ...
          options={[
            ...
            },
            {
              ...
              title: "Chapter text alongside scenes",
            },
          ]}
        />
      </div>
      ...
      <Button
        ...
        size="sm"
        onClick={() =>
          void generate(false, 1 + Math.floor(Math.random() * 976))
        }
        loading={generating}
      >
        <RefreshCw className="w-3.5 h-3.5" />
      ...
      <Button variant="ghost" size="sm" onClick={onExit}>
        <BookOpen className="w-3.5 h-3.5" />
        <span className="hidden sm:inline">Return to reading</span>
      </Button>
    </div>
  );
  ...
  const progressPanel = generating && (
    <div className="panel p-5 sm:p-6 mb-6">
        ...
        <p className="text-sm font-display text-mist-200 truncate flex-1">
          ...
          {genStep === 2 && incoming.length > 0 ? (
            <span className="text-gold-300">
              {" "}
              — scene {Math.min(incoming.length, estimatedSceneCount)} of{" "}
              {estimatedSceneCount} written…
            </span>
          ) : (
      ...
      <Progress
        value={((genStep + genFrac) / CINEMA_STEPS.length) * 100}
        className="mb-4"
      />
      <p className="text-[11px] text-mist-600 leading-relaxed">
        Ankaa is staging the chapter — enlarging the prose into atmosphere,
        light and subtext rather than compressing it.
        {incoming.length > 0 && (
          <span className="text-ankaa-300">
            {" "}
            {incoming.length} scene{incoming.length === 1 ? "" : "s"} landed…
          </span>
        )}
        <span className="block mt-1.5">
          Navigate freely — this run is bound to “{chapter?.title}”.
        </span>
      </p>
    </div>
  );
  ...
  const chapterPanelContent = (
    <div className="panel p-5 sm:p-6 lg:sticky lg:top-6">
      <div className="flex items-center gap-2 mb-4">
        ...
        <span className="text-[10px] font-display uppercase tracking-[0.18em] text-mist-500">
          Source chapter
        </span>
      </div>
      <h2 className="font-garamond italic text-xl sm:text-2xl text-mist-100 mb-5 leading-tight">
        {chapter?.title}
      </h2>
      <div className="space-y-4 max-h-[70vh] overflow-y-auto scrollbar-thin pr-1 font-literata text-[15px] leading-relaxed text-mist-300">
        ...
        {chunks.length === 0 && (
          <p className="text-mist-500 italic">
    ...
    <>
              ...
              >
                ...
                >
                  <div
                    ...
                    style={{ background: moodGradient(s.mood) }}
                  />
                ...
                <div className="p-5 sm:p-6 flex-1 flex flex-col">
                  <p className="text-xs text-mist-500 font-display tracking-wide mb-3">
                    {s.mood}
                  </p>
                  {s.characters.length > 0 && (
                    <div className="flex flex-wrap gap-1.5 mb-4">
                      {s.characters.map((c) => (
                        <span
                          key={c}
                          className="text-[11px] font-display px-2 py-0.5 rounded-full border border-ink-600 bg-ink-800 text-mist-200"
                        >
                          {c}
                        </span>
                      ))}
                  ...
                  <p className="text-[15px] leading-relaxed text-mist-300 font-literata flex-1">
                    {s.body}
                  </p>
                  ...
                  <div className="mt-5 pt-4 border-t border-ink-700/60 flex items-center gap-2 flex-wrap">
                    <Button
                      ...
                      title={
                        onExitToChunk
                          ? "Open the source passage in the reader"
                    ...
                    >
                      <RefreshCw className="w-3.5 h-3.5" />
                      {isRegen ? "Rewriting…" : "Regenerate scene"}
                    </Button>
                  </div>
                  ...
                  {isExpanded && sourceExcerpt && (
                    <p className="mt-3 text-xs text-mist-400 italic font-literata leading-relaxed border-l-2 border-gold-700/50 pl-3">
                      “{sourceExcerpt}”
                      <span className="block mt-1 not-italic text-[10px] font-display uppercase tracking-[0.14em] text-mist-600">
                        passage {sourceChunkIdx + 1} of {chunks.length}
                      </span>
                    </p>
                  )}
                </div>
      ...
      >
        ...
        <Button
          variant="outline"
          size="sm"
          disabled={chapterIndex >= doc.contentJson.chapters.length - 1}
          onClick={onNext}
        >
    ...
    <div className={cx("mx-auto", isSplit ? "max-w-6xl" : "max-w-3xl")}>
      <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
        <div className="min-w-0">
          <p className="text-[11px] font-display uppercase tracking-[0.24em] text-gold-400 mb-1.5">
            Scene view · chapter {chapterIndex + 1}
          </p>
          <h1 className="font-garamond italic text-2xl sm:text-3xl text-mist-100 wrap-break-word">
            {chapter?.title}
          </h1>
        </div>
        {headerActions}
  ...
  return (
    ...
    >
      <div className="p-4 sm:p-5 space-y-6">
        <section>
          <h3 className="text-[10px] font-display uppercase tracking-[0.2em] text-mist-500 mb-3">
            Theme
          </h3>
          <Segmented
          ...
          />
          <p className="text-[11px] text-mist-600 mt-2.5 leading-relaxed">
            Reader themes are independent of the app’s vellum theme.
          </p>
        </section>
        ...
        <section>
          ...
          <div className="mt-3 space-y-1">
            ...
            <Slider
              ...
              max={25}
              step={1}
              unit={rs.letterSpacing ? ` ${rs.letterSpacing / 100}em` : ""}
          ...
          <div
            className="hairline rounded-xl p-4 mt-3 relative overflow-hidden"
            style={{
          ...
          >
            <span className="absolute top-2 right-2 text-[9px] font-display uppercase tracking-[0.16em] text-mist-600">
              preview
            </span>
            The lamp kept its post, and the threshold became what thresholds are
            secretly for.
          </div>
        </section>
          ...
          >
            {ACCENTS.map((a) => (
              <button
                key={a.id}
                role="radio"
                aria-checked={rs.accent === a.id}
                aria-label={a.label}
                title={a.label}
                onClick={() => setReader({ accent: a.id })}
                className={cx(
        ...
        <section className="space-y-1">
          ...
          <Toggle
            label="Focus mode"
            hint="Dim surrounding paragraphs; nothing is hidden."
            checked={rs.focusMode}
            onChange={(v) => setReader({ focusMode: v })}
          />
          <div className="py-2">
            ...
            <Segmented
              ariaLabel="Contrast"
              value={rs.contrast}
              onChange={(v) => setReader({ contrast: v })}
              options={[
          ...
          <Toggle
            label="Motion"
            hint="Disable decorative animation across the app."
            checked={rs.motion}
          ...
          <Toggle
            ...
            hint="Show shortcut affordances in the reader."
            checked={rs.kbdHints}
...
export function ChapterIndexSheet({
  open,
  onClose,
  doc,
  chapterIndex,
      ...
      <div className="p-3 sm:p-4">
        {tab === "chapters" && (
          <ul className="space-y-1">
            {doc.contentJson.chapters.map((ch, i) => {
              const words = ch.chunks.reduce(
                (a, c) => a + c.text.split(/\s+/).length,
                0,
              );
                    ...
                    <span className="min-w-0 flex-1">
                      <span
                        className={cx(
                          ...
                          i === chapterIndex
                            ? "text-gold-300 font-display"
                            : "text-mist-300",
                        )}
                      >
                        {ch.title}
                      </span>
                      <span className="block text-[11px] text-mist-600 mt-0.5">
                        {ch.chunks.length} passages · {words.toLocaleString()}{" "}
                        words
                      </span>
                    </span>
        ...
        {tab === "bookmarks" &&
          (bookmarks.loading ? (
            ...
            <p className="text-sm text-mist-500 px-3 py-8 text-center leading-relaxed">
              No bookmarks yet. Press <kbd>b</kbd> while reading.
            </p>
          ) : (
            <ul className="space-y-2.5">
              {(bookmarks.data ?? []).map((b) => (
                <li
                  key={b.id}
                  className="hairline rounded-xl bg-ink-875 p-3.5 hover-lift"
                >
                  <div className="flex items-center justify-between gap-2 mb-2">
                    <p className="text-sm text-gold-300 font-display truncate">
                      {b.label || "Bookmark"}
                    </p>
                    <IconBtn
                  ...
                  <p className="text-xs text-mist-500 italic font-literata line-clamp-2 leading-relaxed">
                    “{chunkText(b.chunkIndex)}”
                  </p>
                  {b.note && (
                    <p className="text-xs text-mist-400 mt-2 leading-relaxed">
                      {b.note}
                    </p>
                  )}
                  <Button
        ...
        {tab === "notes" &&
          (annotations.loading ? (
            ...
            <p className="text-sm text-mist-500 px-3 py-8 text-center leading-relaxed">
              Select any passage while reading to annotate it.
            </p>
          ) : (
            <ul className="space-y-2.5">
              {(annotations.data ?? [])
                ...
                .map((a) => (
                  ...
                  >
                    ...
                    <p className="text-xs text-mist-300 italic font-literata line-clamp-2 leading-relaxed">
                      “{a.text}”
                    </p>
                    {a.note && (
                      <p className="text-xs text-mist-400 mt-2 leading-relaxed">
