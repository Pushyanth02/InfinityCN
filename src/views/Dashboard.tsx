...
function chapterOf(doc: DocumentRow): number {
  ...
  for (let i = 0; i < doc.contentJson.chapters.length; i++) {
    acc += doc.contentJson.chapters[i]?.chunks.length ?? 0;
...
function minutesLeft(doc: DocumentRow): number {
  return Math.max(
    1,
    Math.round(((100 - doc.readingProgress) / 100) * (doc.wordCount / 230)),
  );
}
    ...
    <Reveal delay={0.04}>
      <div className="relative mt-6 sm:mt-8 overflow-hidden rounded-xl border border-gold-700/40 shadow-glow-gold">
        ...
        <div
          ...
          style={{
            background:
              "linear-gradient(120deg, rgba(217,173,82,0.10) 0%, rgba(109,132,232,0.06) 50%, rgba(219,129,76,0.08) 100%)",
          }}
        />
        <div className="relative flex flex-col sm:flex-row gap-5 sm:gap-6 p-5 sm:p-6 lg:p-7">
          ...
          <div className="flex items-center gap-3 sm:flex-col sm:items-start sm:gap-2 shrink-0">
            ...
            <span className="text-[10px] font-display uppercase tracking-[0.18em] text-mist-600 sm:mt-1">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-sm text-mist-400 leading-relaxed max-w-2xl">
              Luma, Ouro and Ankaa are answering you right now with the grounded{" "}
              <span className="text-mist-300">Anchor engine</span> — extractive,
              on-device, always private. Bring an OpenRouter key and they'll{" "}
              <span className="text-gold-300">speak with live voices</span>:
              streaming answers, seminar-grade study sets, and long-form drafts.
              It takes a minute, costs nothing to try, and the key stays only in
              this tab.
            </p>
            <div className="mt-4 flex flex-wrap items-center gap-3">
              ...
              <button
                onClick={dismiss}
                className="text-xs font-display text-mist-500 hover:text-mist-300 transition-colors min-h-9 px-2"
              >
                I know — hide this
              </button>
            </div>
...
export default function Dashboard() {
  const { go, openDoc } = useNav(
    useShallow((s) => ({ go: s.go, openDoc: s.openDoc })),
  );
  ...
  const current = useMemo(
    () =>
      ready
        .filter((d) => d.readingProgress > 0 && d.readingProgress < 99.5)
  ...
  if (docsQ.loading) {
    return (
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10 sm:py-12">
        ...
        <Skeleton className="h-14 w-104 max-w-full mb-10" />
        ...
        <div className="flex flex-col gap-6 sm:flex-row sm:flex-wrap sm:items-end sm:justify-between">
          <div className="min-w-0">
            ...
            <p className="mt-3 sm:mt-4 text-sm sm:text-[15px] text-mist-400 max-w-xl leading-relaxed">
              Documents become living experiences here — import, read, annotate,
              and let three companions keep you company in the margins.
            </p>
          </div>
          <div className="flex flex-wrap items-center gap-2.5 sm:gap-3 shrink-0">
            ...
            >
              ...
              {connected ? "companions online" : "companions offline"}
            </Badge>
            <Button
              variant="gold"
              onClick={() => go("upload")}
        ...
        <Reveal delay={0.08} className="mt-10 sm:mt-12">
          <Panel className="p-6 sm:p-10">
            <EmptyState
              ...
              action={
                <Button variant="gold" onClick={() => go("upload")}>
                  <Upload className="w-4 h-4" />
          ...
          <Reveal delay={0.06} className="mt-10 sm:mt-12">
            {hero ? (
              <article className="relative panel rounded-xl overflow-hidden hover-lift">
                ...
                >
                  <div
                    className="absolute -top-24 -left-16 w-105 h-105 rounded-full"
                    style={{
                      background:
                        "radial-gradient(circle, var(--acc-soft), transparent 62%)",
                    }}
                  />
                  <div
                    className="absolute right-0 bottom-0 w-1/2 h-full opacity-[0.18]"
                    style={{
                      background: hero.coverGradient,
                      maskImage:
                        "linear-gradient(105deg, transparent 30%, black 90%)",
                      WebkitMaskImage:
                        "linear-gradient(105deg, transparent 30%, black 90%)",
                    }}
                  />
                ...
                <div className="relative flex flex-col md:flex-row gap-6 sm:gap-8 p-5 sm:p-7 lg:p-9">
                  ...
                  <div className="flex items-center gap-4 md:block md:items-start md:gap-0">
                    <div className="relative shrink-0">
                      <CoverArt
                        doc={hero}
                        className="w-28 sm:w-40 lg:w-44 h-40 sm:h-56 lg:h-60 shadow-lift"
                      />
                      <div className="absolute -bottom-4 -right-3 sm:-bottom-5 sm:-right-5">
                        <ProgressRing
                          value={hero.readingProgress}
                          size={56}
                          stroke={5}
                          tone={hero.readingProgress >= 99.5 ? "ok" : "gold"}
                        />
                      </div>
                    </div>
                  </div>
                  <div className="min-w-0 flex-1 md:pt-1">
                    <div className="flex flex-wrap items-center gap-2.5 mb-3">
                      ...
                      <span className="text-[11px] text-mist-600 flex items-center gap-1">
                        ...
                        {hero.lastReadAt
                          ? timeAgo(hero.lastReadAt)
                          : "not opened yet"}
                      </span>
                    </div>
                    <h2 className="font-garamond italic text-2xl sm:text-3xl lg:text-4xl text-mist-100 leading-snug">
                      {hero.title}
                    </h2>
                    ...
                    <dl className="mt-5 sm:mt-6 grid grid-cols-3 gap-3 sm:gap-6 sm:flex sm:flex-wrap sm:gap-x-8 sm:gap-y-3">
                      <div>
                        <dt className="text-[10px] font-display uppercase tracking-[0.18em] text-mist-600">
                          Chapter
                        </dt>
                      ...
                      <div>
                        ...
                        <dd className="font-display text-lg sm:text-xl text-mist-100 tabular-nums mt-0.5">
                          {fmtWords(hero.wordCount).replace(" words", "")}
                          <span className="text-mist-500 text-sm"> words</span>
                        </dd>
                      </div>
                      <div>
                        <dt className="text-[10px] font-display uppercase tracking-[0.18em] text-mist-600">
                        ...
                        <dd className="font-display text-lg sm:text-xl text-mist-100 tabular-nums mt-0.5">
                          ~{minutesLeft(hero)}
                          <span className="text-mist-500 text-sm"> min</span>
                        </dd>
                      </div>
                    </dl>
              ...
              <Panel className="p-6 sm:p-8 flex flex-wrap items-center justify-between gap-5">
                <div>
                  <h2 className="font-display text-xl text-mist-100">
                    {finished.length === ready.length
                      ? "Everything on the shelf is finished."
                      : "Nothing mid-read right now."}
                  </h2>
                  <p className="text-sm text-mist-500 mt-1.5">
                    Pick something from the shelf below and settle in.
                  </p>
                </div>
          ...
          <Reveal delay={0.1} className="mt-12 sm:mt-14">
            <div className="flex items-end justify-between gap-4 mb-5">
              <div className="flex items-center gap-2.5">
                ...
                <h3 className="font-display font-semibold text-lg sm:text-xl text-mist-100">
                  Your shelf
                </h3>
              </div>
              <button
                onClick={() => go("library")}
                className="text-xs font-display uppercase tracking-[0.16em] text-gold-400 hover:text-gold-300 inline-flex items-center gap-1.5 transition-colors min-h-10"
              >
                All {ready.length}
              ...
              {[...current, ...upNext, ...finished].slice(0, 10).map((d, i) => (
                ...
                >
                  <div className="relative transition-transform duration-300 ease-out group-hover:-translate-y-2 group-hover:rotate-[-1.5deg] group-focus-visible:-translate-y-2">
                    <CoverArt
                      doc={d}
                      className="w-28 sm:w-32 h-40 sm:h-44 shadow-lift"
                    />
                    <div className="absolute inset-x-2 bottom-2">
                      <div className="h-1 rounded-full bg-ink-950/60 overflow-hidden">
                        <div
                          className="h-full rounded-full bg-gold-400"
                          style={{ width: `${d.readingProgress}%` }}
                        />
                      </div>
                    </div>
                  </div>
                  <p className="mt-2.5 text-xs font-display text-mist-300 leading-snug line-clamp-2 group-hover:text-gold-300 transition-colors">
                    {d.title}
                  </p>
                  <p className="text-[10px] text-mist-600 mt-0.5">
                    {d.readingProgress >= 99.5
                      ? "finished"
                      : d.readingProgress > 0
            ...
            <Reveal delay={0.05}>
              <Panel className="p-5 sm:p-7">
                <div className="flex items-center justify-between mb-5 sm:mb-6">
                  <h3 className="font-display font-semibold text-base sm:text-lg text-mist-100">
                    Library at a glance
                  </h3>
                  <button
                    onClick={() => go("analytics")}
                    className="text-xs font-display uppercase tracking-[0.16em] text-gold-400 hover:text-gold-300 inline-flex items-center gap-1.5 transition-colors min-h-9"
                  >
                ...
                <div className="grid grid-cols-2 gap-y-6 sm:gap-y-8 gap-x-4">
                  ...
                  <Glance
                    ...
                    sub={
                      ready.length
                        ? `${pct(finished.length, ready.length)}% of shelf`
                        : undefined
                    }
                  />
                  <Glance
                ...
                <div className="mt-6 sm:mt-8 pt-5 sm:pt-6 border-t border-ink-700 flex items-center gap-3">
                  ...
                  <p className="text-xs text-mist-500 leading-relaxed flex-1">
                    {connected
                      ? "OpenRouter key set — companions answer with live models, metered daily."
                      : "Companions are running grounded Anchor engines. Add your OpenRouter key in Settings for live models."}
                  </p>
                  <Button
                    size="xs"
                    variant="outline"
                ...
                {actQ.loading ? (
                  ...
                  <div className="flex flex-col items-center justify-center text-center py-8">
                    ...
                    <p className="text-xs text-mist-600 mt-1">
                      Open a book and the room will keep a diary.
                    </p>
                  </div>
                ) : (
                  <div className="relative">
                    <span
                      className="absolute left-3.75 top-2 bottom-2 w-px bg-ink-700"
                      aria-hidden
                    />
...
function Glance({
  ...
  return (
    <div className="min-w-0">
      ...
      <p className="font-display font-semibold text-[1.6rem] sm:text-[2rem] leading-none text-mist-100 tabular-nums mt-2 truncate">
        {value}
      </p>
