...
const sel = {
    ...
    insights: (s: ReturnType<typeof useStore.getState>) => s.insights,
    chapterTitle: (s: ReturnType<typeof useStore.getState>) => s.chapterTitle,
...
const LibraryCard: React.FC<LibraryCardProps> = ({ chapter, index, onLoad, onDelete }) => (
    <motion.li
        ...
        onKeyDown={e => {
            if ((e.key === 'Enter' || e.key === ' ') && chapter.id) {
                e.preventDefault();
                onLoad(chapter.id);
            }
        }}
...
function App() {
    ...
    const insights = useStore(sel.insights);
    const chapterTitle = useStore(sel.chapterTitle);
    ...
    const loadChapter = useCallback(
        (id: number) => {
            db.chapters
                ...
                .catch(err => {
                    console.error('[DB] Failed to load chapter:', err);
                });
        },
    ...
    const deleteChapter = useCallback(
        async (e: React.MouseEvent | React.KeyboardEvent, id: number) => {
            e.stopPropagation();
                    ...
                    <div className="header-actions">
                        ...
                        <div className="ai-trigger-container">
                            ...
                            >
                                ...
                                />
                                <span
                                    className={`ai-trigger-label ai-trigger-label--${aiProvider}`}
                                >
                                    {aiLabel}
                                </span>
                            </button>
                            <div
                                className={`ai-trigger-indicator ai-trigger-indicator--${aiProvider}`}
                            />
                        </div>
                        <ThemeStudio />
                    </div>
                    ...
                    {hasReader ? (
                        ...
                        >
                            ...
                            <div className="hero-section">
                                ...
                                >
                                    Drop a novel PDF or TXT and experience it in breathtaking,
                                    cinematic typography. Powered by local and on-device AI for
                                    intelligent codexes, recaps, and mood enhancement.
                                </motion.p>
                            </div>
                                ...
                                >
                                    <div className="library-section-header">
                                        ...
                                        <span
                                            ...
                                            aria-label={`${savedChapters.length} saved chapters`}
                                        >
