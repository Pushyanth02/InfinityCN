...
import { PageFade, ToastHost, ErrorBoundary, PageLoader } from "./components/ui";
...
export default function App() {
  ...
  /* first run: seed sample texts; recover jobs interrupted by a reload;
     start the periodic reaper that cleans up finished jobs */
  useEffect(() => {
    void ensureSeed();
  ...
  useEffect(() => {
    const onError = (e: ErrorEvent) => {
      ...
      useToasts.getState().push("error", `Something went wrong: ${e.message || "unknown error"}. Your library is safe — try the action again.`);
    };
    const onRejection = (e: PromiseRejectionEvent) => {
      const msg = e.reason instanceof Error ? e.reason.message : "a background task failed";
  ...
  const pageKey = insightsView ? "insights" : view === "reader" ? `reader-${docId}` : view;

  return (
    <div className={cx("min-h-screen flex flex-col", !motionOn && "motion-off")} data-ring={ring} data-accent={accent}>
      <a href="#main" className="sr-only focus:not-sr-only focus:absolute focus:top-2 focus:left-2 focus:z-[99] focus:px-4 focus:py-2 focus:bg-gold-500 focus:text-[var(--acc-ink)] focus:rounded font-display text-sm">
        Skip to content
      </a>
