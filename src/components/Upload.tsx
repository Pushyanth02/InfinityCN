...
export const Upload: React.FC<UploadProps> = ({ onFileSelect, isLoading }) => {
    ...
    const handleFile = useCallback(
        (file: File) => {
            ...
            if (file.size > MAX_FILE_BYTES) {
                setSizeError(
                    `File too large (${(file.size / (1024 * 1024)).toFixed(1)} MB). Max is ${MAX_FILE_SIZE_MB} MB.`,
                );
                return;
            }
    ...
    return (
        <div>
            ...
            >
                <div className="upload-eyebrow" aria-hidden="true">
                    Source Text
                </div>
                ...
                <div className="upload-title" aria-hidden="true">
                    ...
                    {isBusy ? 'Parsing Fragment…' : droppedName ? droppedName : 'Drop Source Text'}
                </div>
