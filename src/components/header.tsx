...
export function AppHeader() {
  const { view, docId, go, back } = useNav(
    useShallow((s) => ({
      view: s.view,
      docId: s.docId,
      go: s.go,
  ...
  const crumb =
    view === "reader" && doc.data ? doc.data.title : VIEW_LABELS[view];
      ...
      <header className="sticky top-0 z-50 border-b border-ink-700/60 bg-ink-900/85 backdrop-blur-xl safe-top">
        <div className="max-w-7xl mx-auto px-3 sm:px-5 lg:px-6 h-14 sm:h-16 flex items-center gap-2 sm:gap-3">
          ...
          >
            ...
            />
            <span className="font-display font-semibold tracking-[0.18em] uppercase text-sm text-mist-100 hidden sm:inline">
              Lemniscate
            </span>
          </button>
          ...
          >
            {links.map((l) => (
              <button
                ...
                aria-current={
                  view === l.view ||
                  (l.view === "analytics" && view === "history")
                    ? "page"
                    : undefined
                }
                className={cx(
                  "px-3 py-1.5 rounded-lg text-[13px] font-display tracking-wide transition-all",
                  view === l.view ||
                    (l.view === "analytics" && view === "history")
                    ? "text-gold-300 bg-gold-500/10 border border-gold-700/30"
                    : "text-mist-400 hover:text-mist-100 hover:bg-ink-750/60 border border-transparent",
                )}
              >
                {l.label}
              </button>
          ...
          <div className="hidden xl:flex items-center gap-1.5 text-xs text-mist-500 min-w-0 max-w-50">
            {view === "reader" && (
              <IconBtn
            ...
            <span className="truncate font-display uppercase tracking-[0.14em] text-[11px] text-mist-400">
              {crumb}
            </span>
            {view === "reader" && doc.data && (
          ...
          {liveJobs.length > 0 && (
            ...
            >
              ...
              <span className="hidden sm:inline truncate max-w-25">
                {liveJobs[0]?.label ?? "Working"}…
              </span>
          ...
          <IconBtn
            ...
            onClick={() => go("settings")}
            active={view === "settings" || view === "account"}
          >
        ...
        <nav className="p-4 flex flex-col gap-1" aria-label="Mobile">
          {links.map((l) => (
            <button
              ...
              className={cx(
                "flex items-center gap-3 px-3.5 py-3 rounded-lg text-sm font-display transition-colors",
                view === l.view
                  ? "text-gold-300 bg-gold-500/10"
                  : "text-mist-300 hover:bg-ink-750",
              )}
            >
          ...
          <button
            ...
            className="flex items-center gap-3 px-3.5 py-3 rounded-lg text-sm font-display text-mist-300 hover:bg-ink-750 transition-colors"
          >
        ...
        <div className="p-4 space-y-3">
          {(jobsQ.data ?? []).length === 0 ? (
            <p className="text-sm text-mist-500 py-8 text-center px-4 leading-relaxed">
              Long-running AI work — Ankaa drafts, deep analyses — appears here
              with live steps, word counts and ETAs, even if you leave the page.
            </p>
          ) : (
              ...
              <div key={j.id} className="hairline rounded-xl bg-ink-875 p-3.5">
                <div className="flex items-center gap-2.5 mb-1.5">
                  ...
                  {typeof j.words === "number" && j.words > 0 && (
                    <span className="text-[10px] font-display text-ankaa-300 tabular-nums shrink-0">
                      {j.words.toLocaleString()} words
                    </span>
                  )}
                </div>
                {j.status === "running" || j.status === "queued" ? (
                  <>
                    ...
                    <p className="text-[11px] text-mist-500">
                      {j.step}
                      {j.etaSec ? ` · ~${fmtClock(j.etaSec)} left` : ""}
                    </p>
                  </>
                ) : j.status === "failed" ? (
                  ...
                  <p className="text-[11px] text-mist-600">
                    Completed{" "}
                    {new Date(j.updatedAt).toLocaleTimeString(undefined, {
                      hour: "2-digit",
                      minute: "2-digit",
                    })}
                  </p>
          ...
          <p className="text-[10px] text-mist-600 flex items-center gap-1.5 pt-1">
            ...
            Finished jobs auto-clear after 30 minutes. Jobs persist across
            navigation and survive reloads.
          </p>
        </div>
      </Sheet>
    </>
  ...
  const onKey = (e: React.KeyboardEvent) => {
    ...
    if (e.key === "Enter") {
      ...
      if (sel < results.length) {
        const doc = results[sel];
        if (doc) openDoc(doc.id);
        onClose();
      } else {
        actions[sel - results.length]?.run();
        onClose();
      }
    }
  };
        ...
        >
          ...
          >
            ...
            <div className="max-h-[55vh] overflow-y-auto p-2">
              <p className="px-3 pt-2 pb-1 text-[10px] font-display uppercase tracking-[0.16em] text-mist-600">
                Library
              </p>
              {results.length === 0 && (
              ...
              {results.map((d, i) => (
                <button
                  ...
                  className={cx(
                    ...
                    sel === i ? "bg-gold-500/10" : "hover:bg-ink-750",
                  )}
                >
                  <span
                    className="w-8 h-10 rounded-md shrink-0 cover-noise relative"
                    style={{ background: d.coverGradient }}
                    aria-hidden
                  />
                  <span className="min-w-0">
                    <span className="block text-sm text-mist-100 truncate">
                      {d.title}
                    </span>
                    <span className="block text-xs text-mist-500 truncate">
                      {d.author} · {d.sourceType.toUpperCase()} ·{" "}
                      {Math.round(d.readingProgress)}%
                    </span>
                  </span>
                </button>
              ))}
              <p className="px-3 pt-3 pb-1 text-[10px] font-display uppercase tracking-[0.16em] text-mist-600">
                Actions
              </p>
              {actions.map((a, i) => (
                <button
                  key={a.label}
                  onMouseEnter={() => setSel(results.length + i)}
                  onClick={() => {
                    a.run();
                    onClose();
                  }}
