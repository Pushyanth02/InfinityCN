...
export async function persistCinematifiedNarrative(
  ...
  await persistParagraphs(narrative.id, r.scenes)
  await persistScenes(narrative.id, r.scenes)
  await persistCharacters(narrative.id, r.characters)
  await persistLocations(narrative.id, r.locations)
  await persistEvents(narrative.id, r.events)
  await persistArcs(narrative.id, r.arcs)
  await persistPeaks(narrative.id, r.peaks)
  return narrative.id
}

async function persistParagraphs(narrativeId: string, scenes: CinematifiedScene[]): Promise<void> {
...
async function persistScenes(narrativeId: string, scenes: CinematifiedScene[]): Promise<void> {
...
async function persistCharacters(narrativeId: string, characters: CinematifiedResult['characters']): Promise<void> {
...
async function persistLocations(narrativeId: string, locations: CinematifiedResult['locations']): Promise<void> {
...
async function persistEvents(narrativeId: string, events: CinematifiedResult['events']): Promise<void> {
  if (!events.length) return
  const sceneRows = await db.scene.findMany({ where: { narrativeId }, orderBy: { index: 'asc' } })
...
async function persistArcs(narrativeId: string, arcs: CinematifiedResult['arcs']): Promise<void> {
...
async function persistPeaks(narrativeId: string, peaks: CinematifiedResult['peaks']): Promise<void> {
